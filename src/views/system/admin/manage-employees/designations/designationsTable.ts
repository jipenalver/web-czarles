...
export function useDesignationsTable() {
  ...
  const itemData = ref<Designation | null>(null)
  ...
  const onUpdate = (item: Designation) => {
    itemData.value = item
    isDialogVisible.value = true
  }
  ...
  const onConfirmDelete = async () => {
    ...
    const { data, error } = await designationsStore.deleteDesignation(deleteId.value)
  ...
  const onLoadItems = async ({ page, itemsPerPage, sortBy }: TableOptions) => {
    ...
    await designationsStore.getDesignationsTable({ page, itemsPerPage, sortBy }, tableFilters.value)
