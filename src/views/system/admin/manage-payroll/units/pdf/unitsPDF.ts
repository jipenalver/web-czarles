...
export function useUnitsPDF() {
  // States
  const formAction = ref({ ...formActionDefault })
  const isPrinting = ref(false)

  // Actions
  const onExportPDF = async (filters: UnitsFilters) => {
    ...
    try {
      ...
      const unitsTableElement = document.getElementById('units-table')

      if (!unitsTableElement) {
        throw new Error('Units table element not found')
      }

      // Store original styles para ma-restore later
      const originalStyles = {
        transform: unitsTableElement.style.transform || '',
        transformOrigin: unitsTableElement.style.transformOrigin || '',
        position: unitsTableElement.style.position || '',
        width: unitsTableElement.style.width || '',
        backgroundColor: unitsTableElement.style.backgroundColor || '',
        display: unitsTableElement.style.display || '',
      }
      ...
      Object.assign(unitsTableElement.style, originalStyles)
  ...
  const generateFilename = (filters: UnitsFilters) => {
    const baseDate = getDateISO(new Date())
    let filename = `${baseDate}-units-report`
