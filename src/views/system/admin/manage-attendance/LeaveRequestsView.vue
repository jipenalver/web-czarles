...
<template>
  <AppLayout :is-with-app-bar-icon="true" @is-drawer-visible="isDrawerVisible = !isDrawerVisible">
    ...
    <template #content>
      <v-container fluid>
        <HeaderPanel
          :header-items="['Attendance Management', 'Leave Requests']"
          header-icon="mdi-account-arrow-left"
          headline="Approve or reject employee leave requests."
        ></HeaderPanel>

        <AttendanceRequestsTable component-view="leave-requests"></AttendanceRequestsTable>
      </v-container>
    </template>
  </AppLayout>
</template>
