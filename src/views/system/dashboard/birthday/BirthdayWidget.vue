...
<template>
  <v-card class="birthday-widget elevation-8" color="gradient-primary">
    <v-card-title class="pa-6 pb-2">
      ...
      <div class="text-caption mt-1">Celebrating amazing people this month!</div>
    </v-card-title>
          ...
          <div class="d-flex align-center">
            ...
            <div>
              ...
              <div class="text-body-2">
                ...
                {{ getBirthdayEmoji(getAge(person.birthdate)) }}
              </div>
              <div class="text-caption">
                {{ getZodiacSign(person.birthdate) }} •
                {{ getAgeMilestone(getAge(person.birthdate)) }}
              </div>
              <div>
                Serving since
                <strong>
                  {{ date.format(person.hired_at, 'fullDate') }}
                </strong>
                <span> • {{ getYearsOfService(person.hired_at) }}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        >
          <template v-slot:[slotItemLastname]="{ item }">
            <div :class="['d-flex align-center pa-2', isToday(item) ? 'birthday-today' : '']">
              ...
              <div>
                <div
                  :class="['font-weight-bold', isToday(item) ? 'text-pink text-h6' : 'text-body-1']"
                >
